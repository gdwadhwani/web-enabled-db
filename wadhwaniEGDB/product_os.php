...
if (isset($_GET['np'])) { // Already been determined.
    ...
    $query = "select COUNT(*) from products_has_operating_system WHERE idOperating_System = $id";
...
// Make the query.
$query = "select p.idProducts, p.p_name, p.p_releasedate, p.p_price, m.m_name, ps.s_name from products as p, manufacturer as m, product_subtype as ps, products_has_operating_system as pos where p.idManufacturer = m.idManufacturer AND p.idProduct_Subtype = ps.idProduct_Subtype AND pos.idProducts = p.idProducts AND pos.idOperating_System = '$id' ORDER BY $order_by LIMIT $start, $display";
...
// Fetch and print all the records.
    ...
    echo '<h2>No Products are listed for this OS</h2>';
}
...
if ($num_pages > 1) {
    ...
    if ($current_page != 1) {
        echo '<a href="product_os.php?s=0&np=' . $num_pages . '&sort=' . $sort . '&id=' .$id .'">First</a> ';
        echo '<a href="product_os.php?s=' . ($start - $display) . '&np=' . $num_pages . '&sort=' . $sort .'&id=' .$id .'">Previous</a> ';
    }

    // Make all the numbered pages.
    for ($i = 1; $i <= $num_pages; $i++) {
        if ($i != $current_page) {
            echo '<a href="product_os.php?s=' . (($display * ($i - 1))) . '&np=' . $num_pages . '&sort=' . $sort .'&id=' .$id .'">' . $i . '</a> ';
        } else {
            echo $i . ' ';
        }
    }
    ...
    if ($current_page != $num_pages) {
        echo '<a href="product_os.php?s=' . ($start + $display) . '&np=' . $num_pages . '&sort=' . $sort .'&id=' .$id .'">Next</a> ';
        echo '<a href="product_os.php?s=' . (($num_pages-1) * $display) . '&np=' . $num_pages . '&sort=' . $sort .'&id=' .$id .'">Last</a>';
    }
